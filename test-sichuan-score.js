...
assert(/class="sichuan-table-board"[^>]*id="sichuanPlayers"/.test(html), '川麻页面必须使用十字牌桌');
assert(/class="sichuan-table-center/.test(html), '川麻牌桌中央必须显示规则与操作提示');
assert(/class="sichuan-name-edit"[^>]*onclick="event\.stopPropagation\(\);openSichuanSetup\(\)"/.test(html), '川麻玩家姓名必须可从牌桌直接修改');
assert(/function\s+sichuanMissingSuitTile/.test(html) && /tileImgSrc\(/.test(html), '定缺标记必须复用真实麻将牌图片');
assert(/class="missing-suit-tile"/.test(html), '川麻玩家卡片必须用牌图展示定缺');
assert(/data-missing-suit="m"[^>]*>[\s\S]*<img|<img[^>]*data-missing-suit-image="m"/.test(html), '定缺选择器必须使用牌图而非纯文字');
assert(/SICHUAN_FAN_EXAMPLES/.test(html), '番型说明必须提供牌例数据');
assert(/class="sichuan-fan-example"/.test(html), '番型选择按钮必须展示牌例图片');
